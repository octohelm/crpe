...
class KubePkgRegistry {
  ...
  Future updateManifests(KubePkg kubePkg) async {
    if (kubePkg.spec.manifests.isNotEmpty) {
      try {
        var manifest = File(path.join(
          workspace.absolute.path,
          "../manifests/${kubePkg.metadata.name}.yaml",
        ));
        ...
        await manifest.writeAsString(({
          ...
          "kind": "HelmChart",
          "metadata": {
            "name": kubePkg.metadata.name,
            "namespace": kubePkg.metadata.namespace,
            "annotations": {
              "crpe.octohelm.tech/createdAt": DateTime.now().toIso8601String(),
            }
          },
          "spec": {
            "targetNamespace": kubePkg.metadata.namespace,
            "jobImage": KubePkg.helmInstaller,
            "chartContent":
                await kubePkg.helmTgz$().transform(base64.encoder).join(),
          }
        }).toYaml());
      } catch (e, stackTrace) {
